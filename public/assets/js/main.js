		...
		// Viewer.
			...
			// Touch gestures.
				_.$viewer
                    ...
                    .on('click', '.thumbnail, .slide .image', function(event) {

                            var $target = $(event.target);

                            // Check if the click is on the full image in the viewer.
                            if ($target.hasClass('image') && _.$body.hasClass('fullscreen')) {
                                var currentSlide = _.slides[_.current];
                                if (currentSlide && currentSlide.url) {
                                    window.location.href = 'qr.blade.php'; // Replace 'your_target_page.html' with the actual URL.
                                }
                            } else {
                                // Handle the click event for the thumbnail (existing code).
                                var $this = $(this);
                                event.preventDefault();
                                event.stopPropagation();
                                if (_.locked)
